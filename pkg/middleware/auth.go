	...
	return func(c *gin.Context) {
		...
		if claims, ok := token.Claims.(jwt.MapClaims); ok {
			...
			if tokenVersionChecker != nil && okUserID {
				// Initialize as 0 if missing or invalid type, though it should be physically present if generated by auth-api
				tokenVersionClaim := 0
				...
				if err != nil {
					// Prepare response for internal error or just unauthorized?
					// Usually if we can't check version, we should fail closed.
					// But let's log it? The middleware signature doesn't allow returning error easily here without aborting.
					// For now, let's treat it as unauthorized or internal error.
					// Given it's a security check, failure to check = failure to authorize.
